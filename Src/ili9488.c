...
{
//	// Added  this code, Gjonpjer
	// 1. Set the address window to the full screen
	    setAddrWindow(0, 0, width - 1, height - 1);

	    // 2. Pre-calculate the 3-byte color values (RGB666 for SPI)
	    // The ILI9488 in SPI mode requires 18-bit color (3 bytes) per pixel
	    uint8_t r = (color & 0xF800) >> 8;
	    uint8_t g = (color & 0x07E0) >> 3;
	    uint8_t b = (color & 0x001F) << 3;

	    // 3. Prepare a buffer for one full line
	    // We use static to avoid stack overflow, and 480 is the max dimension (height)
	    static uint8_t lineBuffer[480 * 3];

	    // Fill the buffer with the color data for one line
	    ...
	    // 5. Transmit the buffer repeatedly for every line of height
	    // We use HAL_SPI_Transmit (blocking) here. It is very fast because
	    // we are sending large chunks (960+ bytes) at once.
	    for (int y = 0; y < height; y++) {
	        HAL_SPI_Transmit(&hspi2, lineBuffer, width * 3, HAL_MAX_DELAY);
	    }
